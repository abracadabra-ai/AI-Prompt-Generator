...
router.post("/save/category", (req, res) => {
  ...
  form.uploadDir = path.join(__dirname, "../../public/upload/category/");
  ...
  form.parse(req, (err, fields, files) => {
    ...
    fs.rename(files.file.filepath, path.join(form.uploadDir, newFilename), (err) => {
      if (err) {
        ...
        let imgPath = '/upload/category/' + newFilename;
...
router.post("/save/case", (req, res) => {
  ...
  //文件存储路径 最后要注意加 '/' 否则会被存在public下
  form.uploadDir = path.join(__dirname, "../../public/upload/case/");
  ...
  form.parse(req, (err, fields, files) => {
    ...
    fs.rename(files.file.filepath, path.join(form.uploadDir, newFilename), (err) => {
      if (err) {
        ...
        let imgName = newFilename;
        let imgPath = '/upload/case/' + newFilename;
